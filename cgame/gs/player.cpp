...
void gplayer_imp::PlayerEnterWorld()
{
	...
	// DRY
	_task_count = 0;
	FinishedTaskList *pFTL = (FinishedTaskList *)_finished_task_list.begin();
	for (unsigned short i = 0; i < pFTL->m_FnshHeader.m_uTaskCount; i++)
	{
		if (pFTL->m_aTaskList[i].m_Mask != 0)
			continue;
		_task_count++;
	}
...
int gplayer_imp::StayInHandler(world *pPlane, const MSG &msg)
{
	...
	case GM_MSG_SUBSCIBE_ENEMY:
	case GM_MSG_UNSUBSCIBE_ENEMY:
...
int gplayer_imp::MessageHandler(world *pPlane, const MSG &msg)
{
	...
	case GM_MSG_QUERY_PLAYER_EQUIPMENT:
	{
		...
		{
			_runner->send_equipment_info(msg.source, *(int *)(msg.content), msg.param, _color_name);
		}
	}
		return 0;
	case GM_MSG_TEAM_PICKUP:
	...
	case GM_MSG_NPC_BE_KILLED:
	{
		...
		_newstats_info._killed_monsters++;
	}
		return 0;
	...
	case GM_MSG_GM_MQUERY_MOVE_POS:
	{
		...
		else if (world_manager::GetInstance()->GetWorldType() == WORLD_TYPE_ARENABATTLE)
		{
			key.target.key_level4 = _plane->w_ins_key.key1;
		}
	...
	case GM_MSG_PLAYER_KILLED_BY_PLAYER:
	{
		...
		_newstats_info._killed_players++;
	}
		return 0;
...
void gplayer_imp::ReceiveGroupExp(const A3DVECTOR &pos, int total_damage, int exp, int sp, int level, size_t count, int npcid, int npctag, int npc_planeindex, float r, const msg_grpexp_t *list)
{
	//首先除去所有已经离开的队员
	int member_damage = 0;
...
void gplayer_dispatcher::embed_item(size_t chip_idx, size_t equip_idx)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::repair_all(size_t cost)
{
	_tbuf.clear();
	using namespace S2C;
	gactive_imp *pImp = (gactive_imp *)_imp;
...
void gplayer_dispatcher::repair(int where, int index, size_t cost)
{
	_tbuf.clear();
	using namespace S2C;
	gactive_imp *pImp = (gactive_imp *)_imp;
...
void gplayer_dispatcher::spend_money(size_t cost)
{
	_tbuf.clear();
	using namespace S2C;
	gactive_imp *pImp = (gactive_imp *)_imp;
...
void gplayer_dispatcher::get_player_money(size_t money, size_t capacity, unsigned int color_name)
{
	_tbuf.clear();
	using namespace S2C;
	gactive_imp *pImp = (gactive_imp *)_imp;
	CMD::Make<CMD::get_own_money>::From(_tbuf, money, capacity, color_name);
	send_ls_msg((gplayer *)(pImp->_parent), _tbuf);
}
...
void gplayer_dispatcher::send_equipment_info(const XID &target, int cs_index, int sid, unsigned int color_name)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer_imp *pImp = (gplayer_imp *)_imp;
	gplayer *pPlayer = (gplayer *)pImp->_parent;
	CMD::Make<CMD::send_equipment_info>::From(_tbuf, pPlayer, pImp->_equip_info.mask, pImp->_equip_info.data, color_name);
	send_ls_msg(cs_index, target.id, sid, _tbuf);
}

void gplayer_dispatcher::equipment_info_changed(uint64_t madd, uint64_t mdel, const void *buf, size_t size, unsigned int color_name)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer_imp *pImp = (gplayer_imp *)_imp;
	gplayer *pPlayer = (gplayer *)pImp->_parent;
	CMD::Make<CMD::equipment_info_changed>::From(_tbuf, pPlayer, madd, mdel, buf, size, color_name);
	AutoBroadcastCSMsg(pImp->_plane, pPlayer->pPiece, _tbuf, pPlayer->ID.id);
}
...
void gplayer_dispatcher::send_team_data(const XID &leader, size_t team_count, size_t data_count, const player_team::member_entry **list)
{
	_tbuf.clear();
	using namespace S2C;
	gactive_imp *pImp = (gactive_imp *)_imp;
...
void gplayer_dispatcher::send_team_data(const XID &leader, size_t team_count, const player_team::member_entry *list)
{
	_tbuf.clear();
	using namespace S2C;
	gactive_imp *pImp = (gactive_imp *)_imp;
...
void gplayer_dispatcher::item_to_money(size_t index, int type, size_t count, size_t price)
{
	_tbuf.clear();
	using namespace S2C;
	gactive_imp *pImp = (gactive_imp *)_imp;
...
void gplayer_dispatcher::pickup_money_in_trade(size_t money)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::receive_money_after_trade(size_t money)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::purchase_item(int type, size_t money, int amount, int slot_amount, int where, int index)
{
	ASSERT(false);
	/*
		_tbuf.clear();
...
void gplayer_dispatcher::self_inventory_data(int where, unsigned char inv_size, const void *data, size_t len)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::self_inventory_detail_data(int where, unsigned char inv_size, const void *data, size_t len)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::exchange_inventory_item(size_t idx1, size_t idx2)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::move_inventory_item(size_t src, size_t dest, size_t count)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::player_drop_item(size_t where, size_t index, int type, size_t count, unsigned char drop_type)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::exchange_equipment_item(size_t index1, size_t index2)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::equip_item(size_t index_inv, size_t index_equip, int count_inv, int count_eq)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::move_equipment_item(size_t index_inv, size_t index_equip, size_t count)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::self_get_property(size_t status_point, const extend_prop &prop, int attack_degree, int defend_degree, int crit_rate, int crit_damage_bonus, int invisible_degree, int anti_invisible_degree, int penetration, int resilience, int vigour, int anti_def_degree, int anti_resist_degree, int damage_reduce, int magic_damage_reduce, int pray_speed, int task_count, int killed_monsters, int killed_players, int death_by_monsters, int death_by_players)
{
	_tbuf.clear();
	using namespace S2C;
	CMD::Make<CMD::self_get_property>::From(_tbuf, status_point, prop, attack_degree, defend_degree, crit_rate, crit_damage_bonus, invisible_degree, anti_invisible_degree, penetration, resilience, vigour, anti_def_degree, anti_resist_degree, damage_reduce, magic_damage_reduce, pray_speed, task_count, killed_monsters, killed_players, death_by_monsters, death_by_players);
	gplayer *pPlayer = (gplayer *)_imp->_parent;
	send_ls_msg(pPlayer, _tbuf);
}

void gplayer_dispatcher::set_status_point(size_t vit, size_t eng, size_t str, size_t agi, size_t remain)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::send_task_var_data(const void *buf, size_t size)
{
	//这里不用tbuf是因为太大了
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::clear_embedded_chip(unsigned short equip_idx, size_t use_money)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::use_item(char where, unsigned char index, int item_type, unsigned short count, const char *arg, size_t arg_size)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::use_item(int item_type, const char *arg, size_t arg_size)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::trashbox_wealth(char is_usertrashbox, size_t money)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::exchange_trashbox_item(int where, size_t idx1, size_t idx2)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::move_trashbox_item(int where, size_t src, size_t dest, size_t delta)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::exchange_trashbox_inventory(int where, size_t idx_tra, size_t idx_inv)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::trash_item_to_inventory(int where, size_t idx_tra, size_t idx_inv, size_t delta)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::inventory_item_to_trash(int where, size_t idx_inv, size_t idx_tra, size_t delta)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::task_deliver_money(size_t amount, size_t money)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::begin_personal_market(int market_id, const char *name, size_t len)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::send_market_name(const XID &target, int cs_index, int sid, const char *name, size_t len)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::player_waypoint_list(const unsigned short *list, size_t count)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::trade_away_item(int buyer, short inv_idx, int type, size_t count)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::send_equip_detail(int cs_index, int cs_sid, int target, const void *data, size_t size)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::send_inventory_detail(int cs_index, int cs_sid, int target, size_t money, unsigned char inv_size, const void *data, size_t size)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::gain_pet(int index, const void *buf, size_t size)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::send_pet_room(pet_data **pData, size_t start, size_t end)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::send_spec_item_list(int cs_index, int cs_sid, int target, int type, void *data, size_t size)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::invisible_obj_list(gobject **ppObject, size_t count)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::send_player_force_data(int cur_force, size_t count, const void *data, size_t data_size)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::send_force_global_data(char data_ready, size_t count, const void *data, size_t data_size)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::online_award_data(int total_award_time, size_t count, const void *data, size_t data_size)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::pet_rebuild_inherit_start(size_t index, int use_time)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::pet_rebuild_inherit_info(int stime, int pet_id, size_t index, int r_attack, int r_defense, int r_hp, int r_atk_lvl, int r_def_lvl)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::pet_rebuild_inherit_end(size_t index)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::pet_evolution_done(size_t index)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::pet_rebuild_nature_start(size_t index, int use_time)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::pet_rebuild_nature_info(int stime, int pet_id, size_t index, int nature)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::pet_rebuild_nature_end(size_t index)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::query_title(int roleid, int count, int ecount, const void *data, size_t data_size, const void *edata, size_t edata_size)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::player_reincarnation(size_t reincarnation_times)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;
...
void gplayer_dispatcher::generalcard_collection_data(const void *buf, size_t size)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::add_generalcard_collection(size_t collection_idx)
{
	_tbuf.clear();
	using namespace S2C;
...
void gplayer_dispatcher::body_scale_notify(float scale)
{
	_tbuf.clear();
	using namespace S2C;
	gplayer *pPlayer = (gplayer *)_imp->_parent;

	CMD::Make<CMD::body_scale_notify>::From(_tbuf, pPlayer->ID.id, scale);
	AutoBroadcastCSMsg(_imp->_plane, pPlayer->pPiece, _tbuf, -1, 0);
}
...
void gplayer_imp::OnDeath(const XID &lastattack, bool is_pariah, char attacker_mode, int taskdead)
{
	...
	if (lastattack.type == GM_TYPE_PLAYER)
	{
		_newstats_info._death_by_players++;
	}
	if (lastattack.type == GM_TYPE_NPC)
	{
		_newstats_info._death_by_monsters++;
	}
	...
	if (lastattack.type == GM_TYPE_PLAYER)
	{
		...
		SendTo<0>(GM_MSG_PLAYER_KILLED_BY_PLAYER, lastattack, (attacker_mode & attack_msg::PVP_FEEDBACK_KILL), &data, sizeof(data));
	}
...
int gplayer_imp::PlayerGetItemInfoList(int where, size_t count, unsigned char *list)
{
...
void gplayer_imp::PlayerExchangeInvItem(size_t idx1, size_t idx2)
{
	if (_lock_inventory)
	{
...
void gplayer_imp::PlayerMoveInvItem(size_t src, size_t dest, size_t count)
{
	if (_lock_inventory)
	{
		_runner->error_message(S2C::ERR_INVENTORY_IS_LOCKED);
...
void gplayer_imp::PlayerExchangeTrashItem(int where, size_t idx1, size_t idx2)
{
...
void gplayer_imp::PlayerMoveTrashItem(int where, size_t src, size_t dest, size_t count)
{
	item_list &box = GetTrashInventory(where);
...
void gplayer_imp::PlayerExchangeTrashInv(int where, size_t idx_tra, size_t idx_inv)
{
	if (_lock_inventory)
	{
		_runner->error_message(S2C::ERR_INVENTORY_IS_LOCKED);
...
void gplayer_imp::PlayerTrashItemToInv(int where, size_t idx_tra, size_t idx_inv, size_t count)
{
	if (_lock_inventory)
	{
		_runner->error_message(S2C::ERR_INVENTORY_IS_LOCKED);
...
void gplayer_imp::PlayerInvItemToTrash(int where, size_t idx_inv, size_t idx_tra, size_t count)
{
	if (_lock_inventory)
	{
		_runner->error_message(S2C::ERR_INVENTORY_IS_LOCKED);
...
void gplayer_imp::PlayerExchangeTrashMoney(char is_usertrashbox, size_t inv_money, size_t tra_money)
{
	if (_lock_inventory)
	{
		_runner->error_message(S2C::ERR_INVENTORY_IS_LOCKED);
...
void gplayer_imp::PlayerDropInvItem(size_t index, size_t count, bool isProtected)
{
...
void gplayer_imp::ThrowInvItem(size_t index, size_t count, bool isProtected, unsigned char drop_type, const XID &spec_owner)
{
...
void gplayer_imp::PlayerDropEquipItem(size_t index, bool isProtected)
{
...
void gplayer_imp::ThrowEquipItem(size_t index, bool isProtected, unsigned char drop_type, int throw_count, const XID &spec_owner)
{
	if (world_manager::GetWorldLimit().nothrow_anyway)
		return;
	item_data data;
	...
	//发出更改信息
	CalcEquipmentInfo();
	_runner->equipment_info_changed(0, 1ULL << index, 0, 0, _color_name); //此函数使用了CalcEquipmentInfo的结果
}

void gplayer_imp::PlayerExchangeEquipItem(size_t index1, size_t index2)
{
	...
	_runner->equipment_info_changed(mask_add, mask_del, id, count * sizeof(int), _color_name); //此函数使用了CalcEquipmentInfo的结果
}

void gplayer_imp::PlayerEquipItem(size_t index_inv, size_t index_equip)
{
	...
	if (!type1)
	{
		...
		else
		{
			...
			CalcEquipmentInfo();
			_runner->equipment_info_changed(1ULL << index_equip, 0, &id1, sizeof(id1), _color_name); //此函数使用了CalcEquipmentInfo的结果
	...
	CalcEquipmentInfo();
	_runner->equipment_info_changed(0, 1ULL << index_equip, 0, 0, _color_name); //此函数使用了CalcEquipmentInfo的结果
	IncEquipChangeFlag();
}

void gplayer_imp::PlayerMoveEquipItem(size_t index_inv, size_t index_equip)
{
	if (index_inv >= _inventory.Size() || index_equip >= _equipment.Size())
	{
		_runner->error_message(S2C::ERR_FATAL_ERR);
...
bool gplayer_imp::EquipItem(size_t index_inv, size_t index_equip)
{
...
void gplayer_imp::PlayerDropMoney(size_t amount, bool isProtected)
{
	if (_lock_inventory)
	{
		_runner->error_message(S2C::ERR_INVENTORY_IS_LOCKED);
...
void gplayer_imp::ThrowMoney(size_t amount, bool isProtected)
{
	if (world_manager::GetWorldLimit().nothrow_anyway)
		return;
...
void gplayer_imp::PlayerGetProperty()
{
	_runner->self_get_property(_basic.status_point, _cur_prop,
							   ...
							   _penetration, _resilience, GetVigour(), _anti_defense_degree, _anti_resistance_degree, _damage_reduce, _magic_damage_reduce[0], _skill.GetPraySpeed(),
							   _task_count, _newstats_info._killed_monsters, _newstats_info._killed_players, _newstats_info._death_by_monsters, _newstats_info._death_by_players);
}

void gplayer_imp::PlayerSetStatusPoint(size_t vit, size_t eng, size_t str, size_t agi)
{
...
int gplayer_imp::DispatchCommand(int cmd_type, const void *buf, size_t size)
{
	ASSERT(_commander);
	switch (_player_state)
	{
...
void gplayer_imp::OnPickupMoney(size_t money, int drop_id)
{
	if (drop_id)
	{
...
void gplayer_imp::PickupMoneyInTrade(size_t money)
{
	((gplayer_dispatcher *)_runner)->pickup_money_in_trade(money);
	_trade_obj->PickupMoney(money);
}

void gplayer_imp::PickupItemInTrade(const A3DVECTOR &pos, const void *data, size_t size, bool isTeam, int drop_id)
{
...
void gplayer_imp::PurchaseItem(abase::pair<const item_data *, int> *pItem, size_t size, size_t money, int consume_contrib, int force_contrib)
{
...
void gplayer_imp::OnPickupItem(const A3DVECTOR &pos, const void *data, size_t size, bool isTeam, int drop_id)
{
	if (size < sizeof(item_data))
		return;
	item_data *pData = (item_data *)data;
...
void gplayer_imp::OnHeartbeat(size_t tick)
{
	...
	if (_arena_enter_elapsed_time)
	{
		_arena_enter_elapsed_time--;
		if (_arena_enter_elapsed_time <= 0)
		{
			_arena_battle_id = 0;
			_arena_world_tag = 0;
			_arena_team_id = 0;
...
void gplayer_imp::SendServiceRequest(int service_type, const void *buf, size_t length)
{
...
void gplayer_imp::RemoteAllRepair()
{
	...
	if (count > 0)
	{
		...
		if (cost_adjust > 2e9)
		{
			GLog::log(GLOG_INFO, "REMOTE_REPAIR_ALL user %d,remote Repair, cost exceeds maximum possible", _parent->ID.id);
			return;
		}
...
void gplayer_imp::RepairEquipment(int where, size_t index)
{
	switch (where)
	{
	case IL_INVENTORY:
...
void gplayer_imp::TakeOutItem(const int *id_list, size_t list_count, size_t count)
{
...
bool gplayer_imp::CheckItemExist(int item_id, size_t count)
{
	int rst = 0;
...
bool gplayer_imp::CheckItemExist(int inv_index, int item_id, size_t count)
{
	return IsItemExist(inv_index, item_id, count);
}

bool gplayer_imp::CheckItemExist(const int *id_list, size_t list_count, size_t count)
{
	for (size_t i = 0; i < list_count; i++)
	{
		if (id_list[i] <= 0)
...
bool gplayer_imp::Save(archive &ar)
{
	...
	//保存任务信息
	PlayerTaskInterface task_if(this);
	size_t size = task_if.GetActLstDataSize();
	...
	_newstats_info.Save(ar);
	_arenastats_info.Save(ar);

	ar << _cash_resurrect_times_in_cooldown;

	ar << _arena_battle_id << _arena_team_id << _arena_world_tag << _arena_enter_elapsed_time;
...
bool gplayer_imp::Load(archive &ar)
{
	...
	_newstats_info.Load(ar);
	_arenastats_info.Load(ar);
...
bool gplayer_imp::EmbedChipToEquipment(size_t chip, size_t equip)
{
...
bool gplayer_imp::SharpenEquipment(size_t index, addon_data *addon_list, size_t count, int sharpener_level, int sharpener_gfx)
{
	...
	_runner->equipment_info_changed(1ULL << index, 0, &id, sizeof(id), _color_name); //此函数使用了CalcEquipmentInfo的结果
	IncEquipChangeFlag();
	return true;
}
...
bool gplayer_imp::RechargeEquippedFlySword(size_t index, size_t count)
{
...
bool gplayer_imp::RechargeFlySword(size_t element_index, size_t count, size_t fw_index, int fw_id)
{
...
bool gplayer_imp::PlayerUseItem(int where, size_t inv_index, int item_type, size_t count)
{
	...
	// Interceptation code
	if (item_type == 61801 || item_type == 61802 || item_type == 61803)
	{
		DATA_TYPE dt;
		const TASKDICE_ESSENCE &ess = *(const TASKDICE_ESSENCE *)world_manager::GetDataMan().get_data_ptr(item_type, ID_SPACE_ESSENCE, dt);
		ASSERT(dt == DT_TASKDICE_ESSENCE);

		if (ess.task_lists[0].id != 400000)
		{
			return false;
		}

		if (!CheckAndSetCoolDown(COOLDOWN_INDEX_CONSUME_CUBI_ITEM, 1000))
			return false;

		int rst = 0;
		rst = _inventory.Find(rst, item_type);
		if (rst >= 0)
		{
			item &it = _inventory[rst];
			UpdateMallConsumptionDestroying(it.type, it.proc_type, 1);
			...
			_inventory.DecAmount(rst, 1);
			_runner->player_drop_item(gplayer_imp::IL_INVENTORY, rst, item_type, 1, S2C::DROP_TYPE_USE);
		}

		_mall_cash_offset += ess.task_lists[1].id;
		++_mall_order_id;
...
bool gplayer_imp::PlayerSitDownUseItem(int where, size_t inv_index, int item_type, size_t count)
{
	if (where != IL_INVENTORY && where != IL_EQUIPMENT)
		return false;
	if (count == 0)
...
bool gplayer_imp::PlayerUseItemWithTarget(int where, size_t inv_index, int item_type, char force_attack)
{
	if (where != IL_INVENTORY && where != IL_EQUIPMENT)
		return false;
...
void gplayer_imp::DropItemOnDeath(size_t drop_count_inv, size_t drop_count_equip, const XID &spec_owner)
{
...
void gplayer_imp::DamageItemOnDeath(bool restrict_bind, const XID &killer)
{
	...
	if (data == NULL)
		return;
	if ((size_t)damange_drop_count > data->pile_limit)
...
{
	ASSERT(_subscibe_list.size() || _second_subscibe_list.size() || _third_subscibe_list.size());
	...
	if (_third_subscibe_list.size())
		send_ls_msg(_third_subscibe_list.begin(), _third_subscibe_list.end(), h1.data(), h1.size());
}
...
#define Set(var, cls) var = cls->var
	...
	_newstats_info.Swap(rhs->_newstats_info);
	_arenastats_info.Swap(rhs->_arenastats_info);

	Set(_cash_resurrect_times_in_cooldown, rhs);

	Set(_arena_battle_id, rhs);
	Set(_arena_team_id, rhs);
	Set(_arena_world_tag, rhs);
	Set(_arena_enter_elapsed_time, rhs);

#undef Set
	gactive_imp::Swap(rhs);
}

void gplayer_imp::QueryOtherPlayerInfo1(size_t count, int ids[])
{
...
void gplayer_imp::QueryNPCInfo1(size_t count, int ids[])
{
...
void gplayer_imp::IdentifyItemAddon(size_t index, size_t fee)
{
	if (index >= _inventory.Size())
	{
...
bool gplayer_imp::RefineItemAddon(size_t index, int item_type, int rt_index)
{
	if (index >= _inventory.Size())
		return false;
	item &it = _inventory[index];
	...
	if (rt_index >= 0 && (size_t)rt_index >= _inventory.Size())
		return false;
...
void gplayer_imp::UpdateFactionRelation(int *alliance, size_t asize, int *hostile, size_t hsize, bool notify_client)
{
	_faction_alliance.clear();
...
bool gplayer_imp::StartTravelSession(size_t min_time, size_t max_time, const A3DVECTOR &dest_pos, float speed, int vehicle)
{
...
int gplayer_imp::TransformChatData(const void *data, size_t dsize, void *out_buffer, size_t len)
{
	...
	case CHAT_C2S::CHAT_GENERALCARD_COLLECTION:
	{
		...
		if (!ess || dt != DT_POKER_ESSENCE)
			return 0;
		if (!_generalcard_collection.get((size_t)ess->show_order))
			return 0;
...
bool gplayer_imp::PlayerDestroyItem(int where, size_t index, int item_type)
{
...
void gplayer_imp::SetCoolDownData(const void *buf, size_t size)
{
	if (size == 0)
		return;
...
void gplayer_imp::SetPlayerCharMode(const void *buf, size_t size)
{
...
void gplayer_imp::SetAllCoolDown(int id1, int id2, int cooldown_msec, int except_id)
{
	_cooldown.SetAllCoolDown(id1, id2, cooldown_msec, except_id);
...
void gplayer_imp::InitInstanceKey(int create_time, const void *buf, size_t size)
{
	if (size < sizeof(int) * 2)
	{
...
bool gplayer_imp::GetInstanceKeyBuf(char *buf, size_t size)
{
...
bool gplayer_imp::SetInstanceKeyBuf(const void *buf, size_t size)
{
...
void gplayer_imp::SendAllData(bool detail_inv, bool detail_equip, bool detail_task)
{
	...
	_runner->get_player_money(GetMoney(), _money_capacity, _color_name);
...
int gplayer_imp::OI_GetInventoryDetail(GDB::itemdata *list, size_t size)
{
	return _inventory.GetDBData(list, size);
}

int gplayer_imp::OI_GetEquipmentDetail(GDB::itemdata *list, size_t size)
{
...
void gplayer_imp::GainMoneyWithDrop(size_t inc, bool write_log)
{
...
void gplayer_imp::GainMoney(size_t inc)
{
	size_t newmoney = _player_money + inc;
	ASSERT((int)_player_money >= 0);
	if ((int)newmoney < (int)_player_money || newmoney > _money_capacity)
...
void gplayer_imp::SpendMoney(size_t delta, bool write_log)
{
...
bool gplayer_imp::CheckIncMoney(size_t inc)
{
	size_t newmoney = _player_money + inc;
	if ((int)newmoney < (int)_player_money || newmoney > _money_capacity)
	{
...
bool gplayer_imp::CheckDecMoney(size_t inc)
{
...
bool gplayer_imp::ItemToMoney(size_t inv_index, int type, size_t count, int price)
{
...
bool gplayer_imp::TestCreatePet(size_t pet_id)
{
	pet_data data;
	memset(&data, 0, sizeof(data));
...
bool gplayer_imp::SummonPet(size_t index)
{
...
void gplayer_imp::SendPetCommand(int cur_target, int pet_cmd, const void *buf, size_t size)
{
...
int gplayer_imp::ConvertPetDataToEggData(void *data, size_t size, pet_data *pData, const void *src_temp)
{
	//首先计算数据的大小是否足够
...
bool gplayer_imp::BanishPet(size_t index)
{
	return _petman.BanishPet(this, index);
}

int gplayer_imp::PlayerSummonPet(size_t index)
{
...
int gplayer_imp::ServiceConvertPetToEgg(size_t slot_index)
{
	if (_inventory.GetEmptySlotCount() == 0)
...
int gplayer_imp::PlayerBanishPet(size_t index)
{
	pet_data *pData = _petman.GetPetData(index);
	if (!pData)
		return S2C::ERR_PET_IS_NOT_EXIST;
...
void gplayer_imp::SetPetSlotCapacity(size_t new_size, bool notify)
{
...
int gplayer_imp::ConvertPetToEgg(size_t slot_index)
{
	//将宠物还原成宠物蛋的步骤
	//进行基础检测
	//包裹栏需要有空位
	...
	//检查金钱是否足够
	if (GetMoney() < (size_t)ess->money_restored)
...
void gplayer_imp::DBSetPetData(size_t index, const void *data, size_t size)
{
...
void gplayer_imp::DropMoneyAmount(size_t money, bool isProtected)
{
	bool bCanDrop = _player_state == PLAYER_STATE_NORMAL ||
					_player_state == PLAYER_DISCONNECT ||
					_player_state == PLAYER_SIT_DOWN ||
...
int gplayer_imp::OI_GetTrashBoxDetail(int where, GDB::itemdata *list, size_t size)
{
...
bool gplayer_imp::ReturnWaypoint(int point, bool globalteleport)
{
	abase::vector<TRANS_TARGET_SERV> &waypoint_list = globaldata_gettranstargetsserver();
	for (size_t j = 0; j < waypoint_list.size(); j++)
	{
		if (waypoint_list[j].id == point)
		{
			...
			if (globalteleport)
			{
				...
				switch (waypoint_list[j].world_tag)
				{
				case 137:
					validteleport = _basic.level >= 95 && _basic.sec_level > 7;
					break;
				case 121:
					validteleport = _basic.sec_level > 8;
					break;
				case 122:
					validteleport = _basic.sec_level > 8;
					break;
				case 163:
					validteleport = GetRealmLevel() >= 1;
					break;
				default:
					validteleport = true;
				}
				...
				if (validteleport)
					return LongJump(pos, waypoint_list[j].world_tag);
			}
			else
...
void gplayer_imp::SetExtRestParam(const void *buf, size_t size)
{
...
bool gplayer_imp::ChangePetName(size_t index, const char name[], size_t name_len)
{
...
bool gplayer_imp::IsPetExist(size_t index, int pet_tid)
{
	pet_data *pData = _petman.GetPetData(index);
	return pData != NULL && pData->pet_tid == pet_tid;
}

int gplayer_imp::DyePet(size_t p_index, size_t d_index)
{
	//检查宠物是否存在
...
void gplayer_imp::RemoveAllExpireItems()
{
	...
	if (cei2._remove_count)
	{
		RefreshEquipment();
		CalcEquipmentInfo();
		_runner->equipment_info_changed(0, cei2._remove_mask, 0, 0, _color_name); //此函数使用了CalcEquipmentInfo的结果
		IncEquipChangeFlag();
	}
...
void gplayer_imp::RemoveAllExpireAddon()
{
	...
	if (cei2._changed_count)
	{
		...
		_runner->equipment_info_changed(cei2._changed_mask, 0, os.begin(), os.size(), _color_name); //此函数使用了CalcEquipmentInfo的结果
		IncEquipChangeFlag();
	}
...
bool gplayer_imp::PlayerDoShopping(size_t goods_count, const int *order_list, int shop_tid)
{
	//这里需要添加更多可以购买的状态
...
bool gplayer_imp::CheckItemBindCondition(size_t index, int id, int can_webtrade)
{
...
bool gplayer_imp::CheckBindItemDestory(size_t index, int id)
{
	if (!IsItemExist(index, id, 1))
		return false;
	const item &it = _inventory[index];
...
bool gplayer_imp::DestoryBindItem(size_t index, int id)
{
	ASSERT(IsItemExist(index, id, 1));
	//前面已经确认检查过是否可以进行绑定了
	item it;
...
bool gplayer_imp::CheckRestoreDestoryItem(size_t index, int id)
{
	if (!IsItemExist(index, id, 1))
		return false;
...
bool gplayer_imp::RestoreDestoryItem(size_t index, int id)
{
	ASSERT(IsItemExist(index, id, 1));
...
void gplayer_imp::SendGlobalChat(char channel, const void *msg, size_t size, const void *data, size_t dsize)
{
	if (_player_state != PLAYER_STATE_NORMAL && _player_state != PLAYER_STATE_BIND && _player_state != PLAYER_SIT_DOWN && _player_state != PLAYER_STATE_MARKET)
	{
		return;
...
void gplayer_imp::SendFarCryChat(char channel, const void *msg, size_t size, const void *data, size_t dsize)
{
	if (_player_state != PLAYER_STATE_NORMAL && _player_state != PLAYER_STATE_BIND && _player_state != PLAYER_SIT_DOWN && _player_state != PLAYER_STATE_MARKET)
	{
		return;
...
void gplayer_imp::SendNormalChat(char channel, const void *buf, size_t len, const void *data, size_t dsize)
{
	world *pPlane = _plane;
	if (pPlane == NULL)
		return;
...
void gplayer_imp::SendBattleFactionChat(char channel, const void *buf, size_t len, const void *data, size_t dsize)
{
	gplayer *pPlayer = GetParent();
	_plane->BattleFactionSay(GetFaction(), buf, len, _chat_emote, data, dsize, pPlayer->ID.id, pPlayer->base_info.level);
}

void gplayer_imp::SendCountryChat(char channel, const void *msg, size_t size, const void *data, size_t dsize)
{
...
bool gplayer_imp::ProduceItem4(const recipe_template &rt, int materials[16], int idxs[16], int equip_id, int equip_inv_idx, char &inherit_type, void **pitem, unsigned short &crc, int &eq_refine_level, int &eq_socket_count, int eq_stone_type[], addon_data eq_engrave_addon_list[3], size_t &eq_engrave_addon_count)
{
...
bool gplayer_imp::EngraveItem(const engrave_recipe_template &ert, size_t inv_index)
{
	if (inv_index >= _inventory.Size())
		return false;
	item &it = _inventory[inv_index];
...
bool gplayer_imp::ItemAddonRegen(const addonregen_recipe_template &arrt, size_t inv_index)
{
	if (inv_index >= _inventory.Size())
		return false;
	item &it = _inventory[inv_index];
...
bool gplayer_imp::UseItemWithArg(item_list &inv, int inv_index, int where, int item_type, const char *arg, size_t arg_size)
{
...
bool gplayer_imp::PlayerUseItemWithArg(int where, size_t inv_index, int item_type, size_t count, const char *buf, size_t buf_size)
{
	if (where != IL_INVENTORY && where != IL_EQUIPMENT)
		return false;
	if (count == 0)
...
int gplayer_imp::DyeFashionItem(size_t f_index, size_t d_index)
{
	//检查物品是否存在
...
int gplayer_imp::RefineTransmit(size_t src_index, size_t dest_index)
{
	//检查物品是否存在以及是否合法
...
bool gplayer_imp::ForeignDoShoppingStep1(int goods_id, size_t goods_index, size_t goods_slot)
{
	//这里需要添加更多可以购买的状态
	if (_player_state != PLAYER_SIT_DOWN && _player_state != PLAYER_STATE_NORMAL && _player_state != PLAYER_STATE_BIND)
		return false;
...
bool gplayer_imp::ForeignDoShoppingStep2(int item_id, size_t count, int expire_time, size_t cash_need)
{
	//这里需要添加更多可以购买的状态
	if (_player_state != PLAYER_SIT_DOWN && _player_state != PLAYER_STATE_NORMAL && _player_state != PLAYER_STATE_BIND)
		return false;
	...
	int ocount = count;
	if ((size_t)ocount > (size_t)pItem2->pile_limit)
...
bool gplayer_imp::PlayerEquipTrashboxItem(int where, unsigned char trash_idx, unsigned char equip_idx)
{
	...
	if (!type1)
	{
		...
		CalcEquipmentInfo();
		_runner->equipment_info_changed(1ULL << equip_idx, 0, &id1, sizeof(id1), _color_name); //此函数使用了CalcEquipmentInfo的结果
	...
	_runner->equip_trashbox_item(where, trash_idx, equip_idx);
	CalcEquipmentInfo();
	_runner->equipment_info_changed(0, 1ULL << equip_idx, 0, 0, _color_name); //此函数使用了CalcEquipmentInfo的结果
	IncEquipChangeFlag();
	IncTrashBoxChangeCounter();
	return true;
}

void gplayer_imp::PlayerCheckSecurityPasswd(const char *passwd, size_t passwd_size)
{
...
bool gplayer_imp::PlayerDoDividendShopping(size_t goods_count, const int *order_list, int shop_tid)
{
	//这里需要添加更多可以购买的状态
	if (_player_state != PLAYER_SIT_DOWN && _player_state != PLAYER_STATE_NORMAL && _player_state != PLAYER_STATE_BIND)
		return false;
...
bool gplayer_imp::PlayerChooseMultiExp(int index)
{
	...
	if (index < 0 || (size_t)index >= sizeof(cfg->choice) / sizeof(cfg->choice[0]))
		return false;
...
int gplayer_imp::PlayerJoinForce(int force_id)
{
	...
	{
		if (GetMoney() < (size_t)pCfg->join_money_cost)
			return S2C::ERR_OUT_OF_FUND;
	}
...
void gplayer_imp::PlayerRechargeOnlineAward(int type, size_t count, int *list)
{
...
void gplayer_imp::PlayerExchangeWanmeiYinpiao(bool is_sell, size_t count)
{
	if (_player_state == PLAYER_STATE_MARKET)
	{
	...
	{
		if (!count || count > (size_t)world_manager::GetDataMan().get_item_pile_limit(WANMEI_YINPIAO_ID))
			return;
		if ((float)count * WANMEI_YINPIAO_PRICE > 2e9)
			return;
...
int gplayer_imp::PlayerDecomposeFashionItem(size_t inv_index, int fashion_type, size_t fee, int production_type)
{
	if (inv_index >= _inventory.Size())
		return S2C::ERR_ITEM_NOT_IN_INVENTORY;
	item &it = _inventory[inv_index];
...
int gplayer_imp::WeddingBook(int start_time, int end_time, int scene, int bookcard_index)
{
	...
	if (bookcard_index >= 0)
	{
		if ((size_t)bookcard_index >= _inventory.Size())
			return S2C::ERR_ITEM_NOT_IN_INVENTORY;
...
{
	if (invitecard_index < 0 || (size_t)invitecard_index >= _inventory.Size())
		return S2C::ERR_ITEM_NOT_IN_INVENTORY;
...
int gplayer_imp::FactionFortressSetTechPoint(size_t tech_index)
{
	if (!OI_IsMafiaMaster())
		return S2C::ERR_FACTION_FORTRESS_OP_DENYED;
...
int gplayer_imp::FactionFortressResetTechPoint(size_t tech_index, size_t inv_index, int id)
{
	if (!OI_IsMafiaMaster())
		return S2C::ERR_FACTION_FORTRESS_OP_DENYED;
...
int gplayer_imp::FactionFortressHandInMaterial(size_t inv_index, int id, size_t count)
{
	if (!OI_IsMafiaMember())
		return S2C::ERR_FACTION_FORTRESS_OP_DENYED;
...
int gplayer_imp::FactionFortressMaterialExchange(size_t src_index, size_t dst_index, int material)
{
	if (!OI_IsMafiaMaster())
		return S2C::ERR_FACTION_FORTRESS_OP_DENYED;
...
int gplayer_imp::RemoveSpecItem(unsigned char where, unsigned char index, size_t count, int type)
{
	item_list *plist = NULL;
	if (where <= IL_INVENTORY_END)
		plist = &GetInventory(where);
...
void gplayer_imp::change_elf_property(int index, int value)
{
	if (_cur_elf_info.id != (unsigned int)-1)
	{
		...
		CalcEquipmentInfo();
		_runner->equipment_info_changed(1ULL << item::EQUIP_INDEX_ELF, 0, &id1, sizeof(id1), _color_name); //此函数使用了CalcEquipmentInfo的结果
	}
}
...
bool gplayer_imp::Produce4ChooseExec(const recipe_template &rt, int equip_id, int equip_inv_idx, char inherit_type, void **pItem, unsigned short crc, int eq_refine_level, int eq_socket_count, int eq_stone_type[], addon_data eq_engrave_addon_list[3], size_t eq_engrave_addon_count)
{
...
int gplayer_imp::PlayerEvolutionPet(size_t index, int formula_index)
{
...
int gplayer_imp::PlayerAddPetExp(size_t index, size_t num)
{
	//检查宠物是否存在以及是否能被喂养
	if (index >= pet_manager::MAX_PET_CAPACITY)
		return S2C::ERR_PET_IS_NOT_EXIST;
...
int gplayer_imp::PlayerRebuildPetNature(size_t index, int formula_index)
{
	//检查宠物是否合法
	if (index >= pet_manager::MAX_PET_CAPACITY)
		return S2C::ERR_PET_IS_NOT_EXIST;
...
int gplayer_imp::PlayerRebuildPetInheritRatio(size_t index, int formula_index)
{
	//检查宠物是否合法
	if (index >= pet_manager::MAX_PET_CAPACITY)
		return S2C::ERR_PET_IS_NOT_EXIST;
...
bool gplayer_imp::RebulidPetInheritRatio(int pet_id, size_t index, int formula_index, int &r_attack, int &r_defense, int &r_hp, int &r_atk_lvl, int &r_def_lvl)
{
	if (index >= pet_manager::MAX_PET_CAPACITY)
		return false;
...
void gplayer_imp::AcceptInheritRatioResult(int pet_id, size_t index, int r_attack, int r_defense, int r_hp, int r_atk_lvl, int r_def_lvl)
{
	if (index >= pet_manager::MAX_PET_CAPACITY)
		return;
...
bool gplayer_imp::RebuildPetNature(int pet_id, size_t index, int formula_index, int &nature)
{
	if (index >= pet_manager::MAX_PET_CAPACITY)
		return false;
...
void gplayer_imp::AcceptNatureResult(int pet_id, size_t index, int nature)
{
	if (index >= pet_manager::MAX_PET_CAPACITY)
		return;
...
void gplayer_imp::OnAutoTeamPlayerReady(int leader_id)
{
	...
	if (world_tag == 1 || world_tag == 121 || world_tag == 122 || world_tag == 137 || world_tag == 161 || world_tag == 163)
	{
...
void gplayer_imp::PlayerSetAutoTeamGoal(char goal_type, char op, int goal_id)
{
	...
	if (op > 0 && world_tag != 1 && world_tag != 121 && world_tag != 122 && world_tag != 137 && world_tag != 161 && world_tag != 163)
		return;
	if (goal_id <= 0)
		return;
...
int gplayer_imp::PlayerImproveFlysword(size_t inv_idx, int flysword_id)
{
	...
	if (ess == NULL || dt != DT_FLYSWORD_ESSENCE)
		return S2C::ERR_ITEM_NOT_IN_INVENTORY;
	if (ess->max_improve_level <= 0 || (size_t)ess->max_improve_level > sizeof(ess->improve_config) / sizeof(ess->improve_config[0]))
...
bool gplayer_imp::PlayerDoCashVipShopping(size_t goods_count, const int *order_list, int shop_tid)
{
	//这里需要添加更多可以购买的状态
	if (_player_state != PLAYER_SIT_DOWN && _player_state != PLAYER_STATE_NORMAL && _player_state != PLAYER_STATE_BIND)
		return false;
...
int gplayer_imp::OI_GetPetType()
{
	return (_plantpetman.GetPlantCount() <= 0) ? _petman.GetCurPetType() : 4;
}

void gplayer_imp::ReloadInstance()
{
	if (!CheckCoolDown(COOLDOWM_INDEX_RELOAD_INSTANCE))
	{
		_runner->error_message(S2C::ERR_OBJECT_IS_COOLING);
		return;
	}
	...
	if (IsInTeam())
	{
		_runner->error_message(250);
		return;
	}

	RebuildInstanceKey();

	SetCoolDown(COOLDOWM_INDEX_RELOAD_INSTANCE, RELOAD_INSTANCE_COOLDOWN_TIME);
}
...
void gplayer_imp::SetColorName(unsigned int value)
{
	...
	// if (!CheckAndSetCoolDown(7100, 3000))
	//{
	//	_runner->error_message(S2C::ERR_OBJECT_IS_COOLING);
	//	return true;
	// }
	...
	//_color_name = value;
	//_runner->equipment_info_changed(0, 1ULL << 31, 0, 0, _color_name);
	//_runner->get_player_money(GetMoney(), _money_capacity, _color_name);
}

void gplayer_dispatcher::player_arena_queue_notification(int timestamp_start)
{
	_tbuf.clear();
	using namespace S2C;
	CMD::Make<CMD::arena_queue_notification>::From(_tbuf, timestamp_start);
	gplayer *pPlayer = (gplayer *)_imp->_parent;
	send_ls_msg(pPlayer, _tbuf);
}

void gplayer_dispatcher::player_arena_queue_cancel(int failure_reason)
{
	_tbuf.clear();
	using namespace S2C;
	CMD::Make<CMD::arena_queue_cancel>::From(_tbuf, failure_reason);
	gplayer *pPlayer = (gplayer *)_imp->_parent;
	send_ls_msg(pPlayer, _tbuf);
}

void gplayer_dispatcher::player_arena_queue_confirm_battle(int team_id, int battle_id)
{
	_tbuf.clear();
	using namespace S2C;
	CMD::Make<CMD::arena_queue_confirm_battle>::From(_tbuf, team_id, battle_id);
	gplayer *pPlayer = (gplayer *)_imp->_parent;
	send_ls_msg(pPlayer, _tbuf);
}

void gplayer_dispatcher::player_enter_arena_battleground(int role_in_war, int battle_id, int end_time)
{
	_tbuf.clear();
	using namespace S2C;
	CMD::Make<CMD::enter_arena_battleground>::From(_tbuf, role_in_war, battle_id, end_time);
	gplayer *pPlayer = (gplayer *)_imp->_parent;
	send_ls_msg(pPlayer, _tbuf);
}
void gplayer_dispatcher::player_arena_score_update(int TeamInvaderSize, int TeamDefenderSize, int TeamInvaderScore, int TeamDefenderScore, int count, int *roleid)
{
	_tbuf.clear();
	using namespace S2C;
	CMD::Make<CMD::arena_score_update>::From(_tbuf, TeamInvaderSize, TeamDefenderSize, TeamInvaderScore, TeamDefenderScore, count);
	for (int i = 0; i < count; i++)
	{
		CMD::Make<CMD::arena_score_update>::AddRole(_tbuf, roleid[i]);
	}
	gplayer *pPlayer = (gplayer *)_imp->_parent;
	send_ls_msg(pPlayer, _tbuf);
}
void gplayer_dispatcher::player_arena_battle_result(int InvaderPoints, int InvaderDmg, int InvaderSurvivors, int DefenderPoints, int DefenderDmg, int DefenderSurvivors, int BattleResult, int Winner, int BattleMode, int CountInvader, int CountDefender, const void *data, size_t data_size)
{
	_tbuf.clear();
	using namespace S2C;
	CMD::Make<CMD::arena_end_result>::From(_tbuf, InvaderPoints, InvaderDmg, InvaderSurvivors, DefenderPoints, DefenderDmg, DefenderSurvivors, BattleResult, Winner, BattleMode, CountInvader, CountDefender, data, data_size);
	gplayer *pPlayer = (gplayer *)_imp->_parent;
	send_ls_msg(pPlayer, _tbuf);
}

void gplayer_imp::PlayerArenaQueueEnter(int mode)
{
	if (!IsTeamLeader())
	{
		_runner->error_message(S2C::ERR_NOT_TEAM_LEADER);
		return;
	}

	if (!_cooldown.TestCoolDown(COOLDOWN_INDEX_ARENA_QUEUE_APPLY))
	{
		_runner->error_message(S2C::PLAYER_PVP_COOLDOWN);
		return;
	}
	...
	switch (mode)
	{
	case ARENA_BATTLE_3V3:
		num_members = ARENA_BATTLE_3V3_NUM;
		break;
	case ARENA_BATTLE_6V6:
		num_members = ARENA_BATTLE_6V6_NUM;
		break;
	default:
		_runner->error_message(S2C::ERR_ARENA_TEAM_INVALID_SETTINGS);
		return;
		break;
	}

	if (num_members != GetTeamMemberNum())
	{
		_runner->error_message(S2C::ERR_ARENA_TEAM_INVALID_NUMBER);
		return;
	}

	for (int i = 0; i < GetTeamMemberNum(); i++)
	{
		if (GetTeamMember(i).data.level < ARENA_REQUIRED_LEVEL)
		{
	...
	ONET::Thread::Pool::AddTask(new ArenaJoinQueueJob(this, list, count, ARENA_REQUIRED_LEVEL, mode, teamid, ARENA_TICKET_ITEM));
	return 0;
}

bool gplayer_imp::ArenaJoinStep1(int team_id, int battle_id, int world_tag)
{
	_runner->player_arena_queue_confirm_battle(team_id, battle_id);
	_arena_enter_elapsed_time = ARENA_ELAPSE_TIME_ENTER;
	_arena_battle_id = battle_id;
	_arena_world_tag = world_tag;
	_arena_team_id = team_id;
}
bool gplayer_imp::ArenaJoinStep2()
{
	if (_arena_enter_elapsed_time <= 0)
		return false;
	...
	if (_arena_world_tag == 0)
		return false;

	instance_key key;
	memset(&key, 0, sizeof(key));
	GetInstanceKey(_arena_world_tag, key);
	key.target = key.essence;

	key.target.key_level4 = _arena_battle_id;

	A3DVECTOR pos(0, 0, 0);
	//让Player进行副本传送
	if (world_manager::GetInstance()->PlaneSwitch(this, pos, _arena_world_tag, key, 0) < 0)
	{
		GLog::log(GLOG_INFO, "Player failed PlaneSwitch to enter arena map");
		return false;
	}
	return true;
}
bool gplayer_imp::ArenaJoinStep3() {}