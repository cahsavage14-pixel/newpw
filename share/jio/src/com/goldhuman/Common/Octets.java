...
*/
	...
	static public void main(String arg[])
	{
		...
		x.replace("abc".getBytes());
		x.replace("defghijklmn".getBytes());
		try
		{
			x.replace("0123456789".getBytes("UTF-8"));
		}
		catch (Exception e)
		{
		}
		x.insert(x.size(), "abc".getBytes());
		x.insert(x.size(), "def".getBytes());
		System.out.println(new String(x.getBytes()));
		System.out.println("size = " + x.size());
		Octets y = new Octets("ABC".getBytes());
		x.insert(x.size(), y);
		System.out.println(new String(x.getBytes()));
		Octets z = (Octets)x.clone();
		System.out.println(new String(z.getBytes()));
	}
};
